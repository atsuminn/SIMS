	...
	var methods = {
		...
		init : function(selector, params) {
			...
			for ( var key in methods.__default_params) {
				__params[key] = params && params[key] !== void 0 ? params[key]
						: methods.__default_params[key];
			}
			if (__params.allow_multiple_parent_values) {
				__params.regexp_separator = new RegExp(
						__params.multiple_parent_values_separator);
			}
			params = __params;
			// この親子関係を登録 
		...
		validate_selects : function($parents, $children) {
			...
			for (var arr_i = 0, len = arr.length; arr_i < len; arr_i++) {
				...
				if (0 === $selects.size()) {
					$.error('selector ' + $selects.selector
							+ ': element not found');
				}
				$selects.each(function() {
					if ('SELECT' != $(this).prop('tagName')) {
						$.error('selector ' + $selects.selector
								+ ': includes non-select element(s)');
					}
					if (!$(this).attr('id')) {
			...
			var parent_ids = $parents.map(function() {
				return $(this).attr('id');
			});
		...
		manage_this_relation : function(relation) {
			...
			if (all_parents_selected) {
				...
				relation.$children.each(function() {
					methods.enable_relevant_options($(this), relation,
							parent_selected_values);
				});
			} else {
		...
		enable_relevant_options : function($select, relation,
				parent_selected_values) {
			...
			for (var option_i = 0, len = all_options.length; option_i < len; option_i++) {
				...
				for ( var parent_id in parent_selected_values) {
					...
					if (relation.params.allow_multiple_parent_values) {
						// セパレータ区切りで複数の親にマッチ 
						var relevant_values = relevant_value.toString().split(
								relation.params.regexp_separator);
		...
		disable_all_options : function($select, relation) {
			$select.attr('disabled', 'disabled')
					.val(relation.params.null_value).find('option[value!=""]')
					.remove();
		},
